...
function startSanta() {
	...
	var renderer;
	/* canvas performs more than good enough in any platform, so for now we will ignore webGL
	...
	function Gift(x, y, vx, vy) {
		...
		this.sprite.mousedown = this.sprite.touchstart = function () {
			...
			if (santa.dTheta < 2)
				santa.dTheta *= 1.05;
			score.consecutive++;
			//score += velocity * consecutive / hardness
	...
	function Score() {
		...
		this.text.mousedown = function() {console.log("click");};
